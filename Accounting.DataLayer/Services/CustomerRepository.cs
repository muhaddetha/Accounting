...
namespace Accounting.DataLayer.Services
{
    public class CustomerRepository : ICustomerRepository
    {
        ...
        public List<ListCustomerViewModel> GetNameCustomers(string filter = "")
        {
            ...
            return db.Customers.Where(c => c.FullName.Contains(filter)).Select(c => NewMethod(c)).ToList();
        }

        private static ListCustomerViewModel NewMethod(Customers c)
        {
            return new ListCustomerViewModel()
            {
                CustomerID = c.Customer_ID,
                FullName = c.FullName
            };
        ...
        public bool UpdateCustomer(Customers customer)
        {
            ...
            var local = db.Set<Customers>()
                .Local
                .FirstOrDefault(f => f.Customer_ID == customer.Customer_ID);
            if (local != null)
            {
        ...
        {
            return db.Customers.First(c => c.FullName == name).Customer_ID;
        }
